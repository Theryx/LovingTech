...
const inputCls =
  'w-full rounded-lg border border-brand-grey/30 bg-white px-4 py-2 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-blue'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-brand-grey/20 bg-white p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <select
              value={product.category || 'keyboard'}
        ...
        {condition === 'second_hand' && (
          <div className="mt-3 flex items-start gap-2 rounded-lg bg-amber-50 border border-amber-200 px-4 py-3 text-sm text-amber-800">
            ...
            {t({
              en: 'Second-hand products require at least 2 actual photos of the item. No returns accepted.',
              fr: "Les produits d'occasion nécessitent au moins 2 photos réelles de l'article. Aucun retour accepté.",
            })}
          </div>
        )}
      </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            <label className={labelCls}>
              {t({ en: 'Display Name (fallback)', fr: 'Nom affiché (fallback)' })}
            </label>
            <input
              type="text"
              value={product.name}
              ...
              className={inputCls}
              placeholder="Used if FR/EN names are empty"
            />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className={labelCls}>
                {t({ en: 'Compare-at Price (XAF)', fr: 'Prix barré (XAF)' })}
              </label>
              <input
                type="number"
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {!hasVariants && (
              <div>
                <label className={labelCls}>
                  {t({ en: 'Stock Qty', fr: 'Quantité en stock' })}
                </label>
                <input
                  type="number"
      ...
      <div className="rounded-xl border border-brand-grey/20 bg-white p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-brand-dark">
            {t({ en: 'Variants', fr: 'Variantes' })}
          </h2>
          <button
            onClick={onAddVariantGroup}
            className="flex items-center gap-1 text-sm text-brand-grey transition hover:text-brand-blue"
          >
              ...
              <div key={vi} className="rounded-lg border border-brand-grey/20 p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <button
                    ...
                    className="p-2 text-brand-grey hover:text-brand-orange transition"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                <div className="space-y-2">
                  {variant.options.map((opt, oi) => (
                    ...
                    >
                      <input
                        type="text"
                        value={opt.name}
                        onChange={e => onUpdateVariantOption(vi, oi, { name: e.target.value })}
                        className={inputCls}
                  ...
                  <button
                    ...
                    className="flex items-center gap-1 text-xs text-brand-grey hover:text-brand-blue transition mt-1"
                  >
      ...
      <div className="rounded-xl border border-brand-grey/20 bg-white p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-brand-dark">
            {t({ en: 'Specifications', fr: 'Spécifications' })}
          </h2>
          <button
            onClick={onAddSpec}
            className="flex items-center gap-1 text-sm text-brand-grey transition hover:text-brand-blue"
          ...
          {specKeys.map((key, index) => (
            <div key={index} className="flex items-center gap-3">
              <input
                ...
                className="flex-1 rounded-lg border border-brand-grey/30 bg-white px-3 py-2 text-sm text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-blue"
      ...
      <div className="rounded-xl border border-brand-grey/20 bg-white p-6">
        <h2 className="mb-1 text-lg font-semibold text-brand-dark">
          {t({ en: 'Images', fr: 'Images' })}
        </h2>
        {condition === 'second_hand' && (
          <p className="mb-3 text-xs text-amber-700">
            {t({
              en: 'Min. 2 photos required for second-hand',
              fr: "Min. 2 photos requises pour l'occasion",
            })}
          </p>
        )}
