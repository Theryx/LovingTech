    ...
    <main className="min-h-screen bg-white text-brand-dark">
      ...
      <section className="pt-32 pb-24 px-6 max-w-7xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          ...
          >
            {SORTS.map(s => (
              <option key={s.value} value={s.value}>{language === 'fr' ? s.labelFr : s.labelEn}</option>
            ))}
          </select>
        </div>
        ...
        {filtered.length === 0 ? (
          ...
          <>
            <p className="text-sm text-brand-dark/40 mb-6">
              ...
              {hasActiveFilters && (
                <button onClick={clearFilters} className="ml-3 text-brand-blue hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue rounded">
                  {t({ en: 'Clear filters', fr: 'Effacer les filtres' })}
                </button>
              )}
            </p>
