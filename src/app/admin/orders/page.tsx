  ...
  return (
    <div className="max-w-6xl mx-auto px-4">
      <div className="flex items-center justify-between mb-8">
        ...
        <button
          onClick={exportCSV}
          className="flex items-center gap-2 rounded-lg border border-brand-grey/30 px-4 py-2 text-sm font-medium text-brand-dark hover:bg-brand-grey/10 transition"
        >
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <select
          ...
          className="rounded-lg border border-brand-grey/30 px-3 py-2 text-brand-dark focus:outline-none focus:ring-2 focus:ring-brand-blue"
        >
          ...
          <table className="w-full min-w-[800px]">
            <thead>
              <tr className="border-b border-brand-grey/20">
                {['Référence', 'Produit', 'Client', 'Ville', 'Total', 'Statut', 'Date', ''].map(
                  h => (
                    <th
                      key={h}
            ...
            <tbody>
              {filtered.map(order => {
                ...
                return (
                  <tr
                    key={order.id}
                    className={`border-b border-brand-grey/10 hover:bg-brand-grey/5 transition ${isPending ? 'border-l-4 border-l-brand-orange' : ''}`}
                  >
                    <td className="px-5 py-4 font-mono text-sm font-semibold text-brand-dark">
                      {order.order_ref}
                    </td>
                    <td className="px-5 py-4 text-sm text-brand-dark max-w-[160px] truncate">
                      {order.product_name}
                    </td>
                    ...
                    <td className="px-5 py-4">
                      <Link
                        ...
                        className="p-2 text-brand-grey hover:text-brand-blue transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-blue rounded"
                        aria-label={`Voir commande ${order.order_ref}`}
                      >
