    ...
    <main className="min-h-screen bg-white text-brand-dark flex flex-col items-center justify-center px-6 py-16">
      <div className="w-full max-w-lg">
        ...
        <div className={`rounded-2xl border p-6 mb-6 ${info.color}`}>
          ...
          <p className="mt-2 text-sm opacity-80">{t({ en: STATUS_MESSAGE[status as OrderStatus].en, fr: STATUS_MESSAGE[status as OrderStatus].fr })}</p>
        </div>
        ...
        {order.status_history && order.status_history.length > 0 && (
          <div className="rounded-xl border border-brand-grey/20 bg-white p-5 mb-6">
            <h2 className="text-xs font-semibold uppercase tracking-wider text-brand-dark/40 mb-4">{t({ en: 'Timeline', fr: 'Historique' })}</h2>
              ...
              {[...order.status_history].reverse().map((h, i) => (
                <div key={i} className="flex items-start gap-3 text-sm">
                  ...
                  <div>
                    ...
                    <span className="text-brand-dark/40 ml-2 text-xs">{new Date(h.at).toLocaleString(t({ en: 'en-US', fr: 'fr-FR' }))}</span>
                  </div>
                </div>
              ))}
            </div>
