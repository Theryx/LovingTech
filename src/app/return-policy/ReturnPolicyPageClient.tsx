      ...
      <article className="mx-auto max-w-3xl px-6 pb-24 pt-32">
              ...
              <thead>
                <tr className="border-b border-brand-grey/20 bg-brand-grey/10">
                  <th className="px-4 py-3 text-left font-semibold text-brand-dark">
                    {t({ en: 'Condition', fr: 'État' })}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold text-brand-dark">
                    {t({ en: 'Window', fr: 'Délai' })}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold text-brand-dark">
                    {t({ en: 'Accepted reasons', fr: 'Motifs acceptés' })}
                  </th>
                  <th className="px-4 py-3 text-left font-semibold text-brand-dark">
                    {t({ en: 'Not accepted', fr: 'Non accepté' })}
                  </th>
                </tr>
              </thead>
              <tbody>
                <tr className="border-b border-brand-grey/10">
                  ...
                  <td className="px-4 py-3">
                    {t({
                      en: 'Defective on arrival, wrong item, damaged packaging',
                      fr: "Défectueux à l'arrivée, mauvais article, emballage endommagé",
                    })}
                  </td>
                  <td className="px-4 py-3 text-brand-dark/60">
                    {t({
                      en: 'Change of mind, opened and functional',
                      fr: "Changement d'avis, ouvert et fonctionnel",
                    })}
                  </td>
                </tr>
                <tr className="border-b border-brand-grey/10">
                  ...
                  <td className="px-4 py-3">
                    {t({
                      en: 'Defective, undisclosed fault, wrong item',
                      fr: 'Défectueux, défaut non divulgué, mauvais article',
                    })}
                  </td>
                  <td className="px-4 py-3 text-brand-dark/60">
                    {t({
                      en: 'Customer-caused physical damage',
                      fr: 'Dommages physiques causés par le client',
                    })}
                  </td>
                </tr>
                <tr>
                  <td className="px-4 py-3 font-medium">
                    {t({ en: 'Second-hand', fr: 'Occasion' })}
                  </td>
                  <td className="px-4 py-3 text-red-600 font-medium">
                    {t({ en: 'No returns', fr: 'Aucun retour' })}
                  </td>
                  <td className="px-4 py-3 text-brand-dark/40">
                    {t({ en: 'sold as-is', fr: "vendu en l'état" })}
                  </td>
                  <td className="px-4 py-3 text-brand-dark/60">
                    {t({ en: 'All claims', fr: 'Toutes les réclamations' })}
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <section className="mb-10">
          <h2 className="text-xl font-bold mb-4">
            {t({ en: 'Return process', fr: 'Procédure de retour' })}
          </h2>
          <ol className="space-y-3 text-sm text-brand-dark/70">
            {[
              t({
                en: 'Contact us via WhatsApp +237 655 163 248 within the return window.',
                fr: 'Contactez-nous via WhatsApp +237 655 163 248 dans le délai de retour.',
              }),
              t({
                en: 'Provide: order number + photo or video of the defect.',
                fr: 'Fournissez: numéro de commande + photo ou vidéo du défaut.',
              }),
              t({
                en: 'Admin reviews within 48 hours.',
                fr: "L'administration examine la demande sous 48h.",
              }),
              t({
                en: 'If accepted: replacement or store credit valid 90 days.',
                fr: 'Si accepté: remplacement ou crédit valable 90 jours.',
              }),
              t({
                en: 'If rejected: written explanation via WhatsApp.',
                fr: 'Si refusé: explication écrite via WhatsApp.',
              }),
            ].map((step, i) => (
        ...
        <section className="mb-10">
          <h2 className="text-xl font-bold mb-4">
            {t({ en: 'Void conditions', fr: "Conditions d'annulation" })}
          </h2>
          <ul className="space-y-2 text-sm text-brand-dark/70">
            {[
              t({ en: 'After the return window', fr: 'Après le délai de retour' }),
              t({
                en: 'Physical damage caused after delivery',
                fr: 'Dommages physiques causés après la livraison',
              }),
              t({
                en: 'Missing original packaging (New products)',
                fr: "Emballage d'origine manquant (produits neufs)",
              }),
              t({
                en: 'Second-hand items — no exceptions',
                fr: "Articles d'occasion — aucune exception",
              }),
            ].map((item, i) => (
